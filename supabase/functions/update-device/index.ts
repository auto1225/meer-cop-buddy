...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Sync name ↔ device_name: always keep both columns consistent
    if (typeof fieldsToUpdate.name === "string" && !fieldsToUpdate.device_name) {
      fieldsToUpdate.device_name = fieldsToUpdate.name;
    }
    if (typeof fieldsToUpdate.device_name === "string" && !fieldsToUpdate.name) {
